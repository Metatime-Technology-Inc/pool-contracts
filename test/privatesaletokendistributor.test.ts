...
import { MTC, PrivateSaleTokenDistributor } from "../typechain-types";
...
describe("PrivateSaleTokenDistributor", function () {
    async function initiateVariables() {
        ...
        const privateSaleTokenDistributor = await PrivateSaleTokenDistributor.connect(deployer)
            .deploy(mtc.address, LISTING_TIMESTAMP, LISTING_TIMESTAMP + periodDurationInSeconds) as PrivateSaleTokenDistributor;
    ...
    describe("Create private sale pool & claim period", async () => {
        ...
        it("try to create with wrong constructor params", async function () {
            ...
            await expect(PrivateSaleTokenDistributor.connect(deployer)
                .deploy(ethers.constants.AddressZero, LISTING_TIMESTAMP, LISTING_TIMESTAMP + periodDurationInSeconds)).to.be.revertedWith("PrivateSaleTokenDistributor: invalid token address");

            await expect(PrivateSaleTokenDistributor.connect(deployer)
                .deploy(mtc.address, LISTING_TIMESTAMP + periodDurationInSeconds, LISTING_TIMESTAMP)).to.be.revertedWith("PrivateSaleTokenDistributor: end time must be bigger than start time");
        });
        ...
        it("should initiate private sale & test claim period", async function () {
            ...
            await expect(privateSaleTokenDistributor.connect(deployer).setClaimableAmounts(addrs, amounts))
                .to.be.revertedWith("PrivateSaleTokenDistributor: total claimable amount does not match");
            ...
            await expect(privateSaleTokenDistributor.connect(deployer).setClaimableAmounts([addrs[0]], amounts)).to.be.revertedWith("PrivateSaleTokenDistributor: user and amount list lengths must match");
            
            // try to set address again
            await expect(privateSaleTokenDistributor.connect(deployer).setClaimableAmounts([addrs[0]], [amounts[0]])).to.be.revertedWith("PrivateSaleTokenDistributor: address already set");

            // try to set 0x address and expect to be reverted
            await expect(privateSaleTokenDistributor.connect(deployer).setClaimableAmounts([ethers.constants.AddressZero], [toWei(String(1_000))])).to.be.revertedWith("PrivateSaleTokenDistributor: cannot set zero address");
            ...
            await expect(privateSaleTokenDistributor.connect(user_1).claim()).to.be.revertedWith("PrivateSaleTokenDistributor: tokens cannot be claimed yet");
            ...
            await expect(privateSaleTokenDistributor.connect(deployer).setClaimableAmounts([user_5.address], [toWei(String(1_000_000))]))
                .to.be.revertedWith("PrivateSaleTokenDistributor: claim period has already started");

            // Try to claim tokens with wrong address
            await expect(privateSaleTokenDistributor.connect(user_5).claim())
                .to.be.revertedWith("PrivateSaleTokenDistributor: no tokens to claim");
            ...
            await expect(privateSaleTokenDistributor.connect(deployer).sweep()).to.be.revertedWith("PrivateSaleTokenDistributor: cannot sweep before claim end time");

            // try to claim after claim period end
            await incrementBlocktimestamp(ethers, (SECONDS_IN_A_DAY * 1000) + periodDurationInSeconds);
            await expect(privateSaleTokenDistributor.connect(user_4).claim()).to.be.revertedWith("PrivateSaleTokenDistributor: claim period has ended");

            // try to sweep after claim period end
            await privateSaleTokenDistributor.connect(deployer).sweep();

            // try to sweep again
            await expect(privateSaleTokenDistributor.connect(deployer).sweep()).to.be.revertedWith("PrivateSaleTokenDistributor: no leftovers");
        });
    });
});