...
describe("Trigonometry", function () {
  async function initiateVariables() {
    ...
    const mockTrigonometry = (await MockTrigonometry.connect(
      deployer
    ).deploy()) as MockTrigonometry;
  ...
  describe("Try trigonometric operations", async () => {
    ...
    it("try sin for each angle", async () => {
      const { mockTrigonometry } = await loadFixture(initiateVariables);
      ...
      for (let i = 0; i < angles.length; i++) {
        ...
        const sin_1 =
          Math.floor(
            Number(ethers.utils.formatEther(calculatedSin_1)) * powerOf10
          ) / powerOf10;
        const sin_2 =
          Math.floor(Number(calculatedSin_2) * powerOf10) / powerOf10;
